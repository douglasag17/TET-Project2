...
string toLower(string wordP) {
  ...
  int main (int argc, char *argv[]) {
    ...
    if (rank == 0) {
	  ...
	  for (int i = 0; i < 10; i++) {
	    MPI_Recv(&vectorKeyRec, N, MPI_CHAR, 1, 1, MPI_COMM_WORLD, &info);
	    MPI_Recv(&vectorValRec, N, MPI_CHAR, 1, 1, MPI_COMM_WORLD, &info);
    ...
    if (rank == 2) {
      MPI_Recv(&valueRec, 40, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &info);
      //cout << valueRec << endl;
    }
